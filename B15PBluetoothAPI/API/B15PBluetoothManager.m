...
@implementation B15PBluetoothManager
static  B15PBluetoothManager *B15SManager=nil;
+(B15PBluetoothManager *)sharedInstance
{
    @synchronized (self)
    {
        if (B15SManager == nil)
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
    ...
    if ([self.status isEqualToString:DeviceConnectSuccess])
    {
        if (_delegate&&[_delegate respondsToSelector:@selector(centralConnectDeviceStatus:withBlueToothManager:)])
        {
            ...
            [_delegate centralConnectDeviceStatus:B15SBluetoothManagerConnectSuccess withBlueToothManager:[B15PBluetoothManager sharedInstance]];
        }
    }
    else if ([self.status isEqualToString:DeviceConnectFail])
    {
            ...
            if (_delegate&&[_delegate respondsToSelector:@selector(centralConnectDeviceStatus:withBlueToothManager:)])
            {
                ...
                [_delegate centralConnectDeviceStatus:B15SBluetoothManagerConnectFailed withBlueToothManager:[B15PBluetoothManager sharedInstance]];
            }
    }
...
-(CBPeripheral*)fetchLastConnectPeripheral
{
    ...
    {//根据UUID取回上次连接的设备
        ...
        [[B15PPeripheral sharedInstance]setValue:device forKey:@"peripheral"];
...
-(void)stopScanDevice{
    ...
    if (_delegate&&[_delegate respondsToSelector:@selector(centralDidScanDevice:withBlueToothManager:)])
    {
        ...
        [_delegate centralDidScanDevice:_peripheralsArray withBlueToothManager:[B15PBluetoothManager sharedInstance]];
...
- (void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral
{
    ...
    [[B15PPeripheral sharedInstance]setValue:_peripheral forKey:@"peripheral"];
    [[B15PPeripheral sharedInstance]initPeripheral];
