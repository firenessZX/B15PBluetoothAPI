...
-(void)uploadDevice
{
    if ([[B15PBluetoothManager sharedInstance].peripheral.name isEqualToString:@"DfuLang"])
    {
...
-(void)DFUUpdateVersion
{
    ...
    {
        CBPeripheral *DFUPeripheral=[B15PBluetoothManager sharedInstance].peripheral;
        CBCentralManager *manager=[B15PBluetoothManager sharedInstance].centralManager;
...
-(void)didStateChangedTo:(enum DFUState)state
{
    ...
    switch (state)
    {
        case DFUStateCompleted:
            ...
            [B15PBluetoothManager sharedInstance].autoReconnect=YES;
            break;
        case DFUStateFailed:
            if (_delegate && [_delegate respondsToSelector:@selector(firmwareUpdateStatus:)])
            {
